<template>
  <el-dialog
    ...
    <el-form ref="dataForm" :model="dataForm" label-width="100px">
      ...
      <el-form-item label="目标类型" prop="targetType" hidden="">
        <el-input v-model="dataForm.targetType"/>
      </el-form-item>
...
<script>
import { getComment, review } from '@/api/comment'
...
export default {
  ...
  methods: {
    ...
    handleModifyStatus(status) {
      review({ 'id': this.dataForm.id, 'status': status }).then(response => {
