  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" :default-sort="{prop: 'id', order: 'descending'}" border fit highlight-current-row style="width: 100%" @sort-change="sortChange">
      ...
      <el-table-column align="center" label="评论人类型" width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isAdmin===1? 'danger' :(scope.row.isAdmin===2)? '' : 'warning'">
            {{ scope.row.isAdmin===1? '管理员' :(scope.row.isAdmin===2)? '普通用户' :'审核员' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="状态" width="80">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status===1? 'success' : 'info'">
            {{ scope.row.status===1?'已发布':scope.row.status===2?'未通过':'待审核' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column width="280" align="center" label="操作">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status===1" type="primary" size="mini" icon="el-icon-edit" @click="replyCommentHandle(scope.row.id)">
            回复
          </el-button>
          ...
          <el-button v-if="scope.row.isDelete!==1" size="mini" icon="el-icon-delete" style="margin-left: 10px;" type="danger" @click="deleteComment(scope.row.id)">
            删除
          </el-button>
        </template>
      </el-table-column>
