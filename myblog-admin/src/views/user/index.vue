<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.isAdmin" placeholder="用户身份" clearable class="filter-item" style="width: 110px">
        <el-option :key="1" :value="1" label="管理员" />
        <el-option :key="2" :value="2" label="审核员" />
        <el-option :key="3" :value="3" label="普通用户" />
      </el-select>
    ...
    <el-table v-loading="listLoading" :default-sort = "{prop: 'id', order: 'descending'}" :data="list" border fit highlight-current-row style="width: 100%" @sort-change="sortChange">
      ...
      <el-table-column align="center" label="用户身份" width="100">
        <template slot-scope="scope">
          <el-tag :type="scope.row.isAdmin===1? 'danger' :(scope.row.isAdmin===2)? '' : 'warning'">
            {{ scope.row.isAdmin === 1 ? '管理员' : (scope.row.isAdmin === 2 ? '审核员' : '普通用户') }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column width="320" align="center" label="操作">
        <template slot-scope="scope">
          <template v-if="scope.row.isAdmin===3">
            <el-button size="mini" icon="el-icon-arrow-down" type="info" @click="updateUserType(scope.row.id,1)">设为管理员
            </el-button>
          </template>
          <template v-else-if="scope.row.isAdmin===1">
            <el-button size="mini" icon="el-icon-arrow-up" type="warning" @click="updateUserType(scope.row.id,2)">取消管理员
            </el-button>
          </template>
          <template v-else>
            <el-button size="mini" icon="el-icon-arrow-up" type="warning" @click="updateUserType(scope.row.id,3)">取消审核员
            </el-button>
          </template>
